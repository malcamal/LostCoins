	...
	uint64_t rekey, int nbit, int nbit2, bool paranoiacSeed, const std::string& rangeStart1, const std::string& rangeEnd1, bool& should_exit)
{
	...
	if (rekey == 1) {
		...
		if (nbit2 > 0) {
			printf("  Rotor CPU   : %d cores consistently seek Private key in range\n", nbit2);
		}
		else {
			printf("  Rotor GPU   : Reloading starting hashes every %d.000.000.000 on the counter\n", maxFound);
		}
	...
	if (rekey == 5) {
		...
		printf("\n  Random mode : %.0f \n  Using       : Mnemonic words from file %s \n  List        : %d words \n  Rotor       : Generation of %d random words \n", (double)rekey, seed.c_str(), n, nbit);
			printf("  Site        : https://github.com/phrutis/LostCoins \n  Donate      : bc1qh2mvnf5fujg93mwl8pe688yucaw9sflmwsukz9 \n\n");
	}
...
void LostCoins::getCPUStartingKey(int thId, Int &key, Point &startP)
{
	...
	if (rekey == 5) {
		...
		if (nbit == 2) {
			...
			for (int gz = 0; gz < random; gz++) {
				getline(in, result);
			}
			for (int fr = 0; fr < random2; fr++) {
				getline(in, result2);
			}
		...
		if (nbit == 3) {
			...
			for (int kr = 0; kr < random3; kr++) {
				getline(in, result3);
			}
			std::stringstream ss;
			ss << result << " " << result2 << " " << result3;
			std::string input = ss.str();
			string nos = sha256(input);
			char* cstr = &nos[0];
			key.SetBase16(cstr);
...
void LostCoins::getGPUStartingKeys(int thId, int groupSize, int nbThread, Int *keys, Point *p)
{
   if (rekey == 1) {
		...
		int rangeShowThreasold = 4;
		int rangeShowCounter = 0;
		printf("\n  Divide the range %s into %d cores and threads for quick search  \n", rangeDiff2.GetBase16().c_str(), nbThread);
	 ...
	 for (int i = 0; i < nbThread; i++) {
		...
		if (rekey == 5) {
			...
			if (nbit == 2) {
				...
				for (int jk = 0; jk < random; jk++) {
					getline(in, result);
				}
				for (int kh = 0; kh < random2; kh++) {
					getline(in, result2);
				}
			...
			if (nbit == 3) {
				...
				for (int pj = 0; pj < random; pj++) {
					getline(in, result);
				}
				for (int bi = 0; bi < random2; bi++) {
					getline(in, result2);
				}
				for (int ri = 0; ri < random3; ri++) {
					getline(in, result3);
				}
				std::stringstream ss;
				ss << result << " " << result2 << " " << result3;
	...
	while (isAlive(params)) {
		...
		if (rekey == 1) {
			...
			if (nbit2 > 0) {
				...
				if ((count - lastRekey) > (100000000000 * 1)) {
					// Rekey request
					rekeyRequest(params);
					lastRekey = count;
				}
		...
		if (rekey == 3) {
			...
			if (nbit2 > 0) {
				...
				if ((count - lastRekey) > (472000000 * maxFound)) {
					// Rekey request
					rekeyRequest(params);
					lastRekey = count;
				}
