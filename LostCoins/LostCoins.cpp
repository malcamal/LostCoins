	...
	uint64_t rekey, int nbit, int nbit2, bool paranoiacSeed, const std::string& rangeStart1, const std::string& rangeEnd1, bool& should_exit)
{
	...
	if (rekey == 0) {
		...
		printf("  Rotor       : For large files use -t 11 max (1 core = ~30.000/s, 1 thread = ~5.000/s) \n");
		

		if (nbit2 > 11) {
			printf("  Rotor CPU   : Only works 11 core !!! You are using (%d) cores!!! It will NOT add speed! Multithreading work is only 11 core! USE max -t 11 \n", nbit2);
		}
	...
	if (rekey == 1) {
		...
		if (nbit == 0) {
			printf("  Rotor GPU   : Save checkpoint every (default: 60 minutes) to file LostCoins-Continue.bat Use -n ? (1-1000 minutes)\n");
		}
		else {
			printf("  Rotor GPU   : Save checkpoint every %d minutes to file LostCoins-Continue.bat \n", nbit);
		}
		printf("  Site        : https://github.com/phrutis/LostCoins \n  Donate      : bc1qh2mvnf5fujg93mwl8pe688yucaw9sflmwsukz9 \n\n");
	}
...
void LostCoins::FindKeyCPU(TH_PARAM* ph)
{
	if (rekey == 1) {
		...
		if (thId > 0) {
			printf("  Works correctly only on one core  \n");
		}
	}
	...
	if (rekey == 7) {
		...
		string zhup;
		if (maxFound == 100) {
			dict.Reset("0123456789", 1, 12);
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : 0123456789 \n", (double)rekey);
		}
		if (maxFound == 11) {
			dict.Reset("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 1, 12);
		...
		if (maxFound == 14) {
			dict.Reset("aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ", 1, 12);
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ \n", (double)rekey);
		}
		if (maxFound == 15) {
			dict.Reset("aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789", 1, 12);
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789 \n", (double)rekey);
		}
		if (maxFound == 16) {
			dict.Reset("aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~", 1, 12);
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~ \n", (double)rekey);
		}
		if (maxFound == 17) {
			dict.Reset("aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~ ", 1, 12);
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~(SPACE) \n", (double)rekey);
		}
		if (maxFound == 18) {
			dict.Reset("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя ", 1, 12);
			setlocale(LC_ALL, "Russian");
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя(SPACE) \n", (double)rekey);
		}
		if (maxFound == 19) {
			dict.Reset("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя0123456789", 1, 12);
			setlocale(LC_ALL, "Russian");
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя0123456789 \n", (double)rekey);
		}
		if (maxFound == 20) {
			dict.Reset("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~", 1, 12);
			setlocale(LC_ALL, "Russian");
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~ \n", (double)rekey);
		}
		
		if (maxFound == 21) {
			dict.Reset(seed, 1, 12);
			setlocale(LC_ALL, "Russian");
			printf("\n  Mode        : %.0f \n  Using       : Brute force Slow algorithm -t 1 USE ONLY 1 CPU CORE\n  List        : Chars %s \n", (double)rekey, seed.c_str());
		...
		if (nbit == 7) {
			printf("  Rotor       : <<1-12>>+<<1-12>> \n");
		}
		printf("  Site        : https://github.com/phrutis/LostCoins \n  Donate      : bc1qh2mvnf5fujg93mwl8pe688yucaw9sflmwsukz9 \n\n");
		int thId = ph->threadId;
		counters[thId] = 0;
		
		while (dict.Next(str95)) {
	...
	if (rekey == 8) {
		...
		while (1 < 2) {
			...
			if (maxFound == 120) {
				...
				{
					pass[ihh] = str[rand() % strN];
				}
				int N2 = 1 + rand() % 8;
				char str2[]{ "abcdefghijklmnopqrstuvwxyz" };
				int str2N2 = 26;
				char* pass2 = new char[N2 + 1];
				for (int ijj = 0; ijj < N2; ijj++)
				{
					pass2[ijj] = str2[rand() % str2N2];
				}
				int N3 = 1 + rand() % 8;
				char str3[]{ "abcdefghijklmnopqrstuvwxyz" };
				int str3N3 = 26;
				char* pass3 = new char[N3 + 1];
				for (int iyy = 0; iyy < N3; iyy++)
				{
					pass3[iyy] = str3[rand() % str3N3];
				}
				std::stringstream ss;
				ss << pass << " " << pass2 << " " << pass3;
				std::string input = ss.str();
			...
			if (maxFound == 121) {
				...
				{
					pass[irr] = str[rand() % strN];
				}
				int N2 = 1 + rand() % 8;
				char str2[]{ "abcdefghijklmnopqrstuvwxyz" };
				int str2N2 = 26;
				char* pass2 = new char[N2 + 1];
				for (int ier = 0; ier < N2; ier++)
				{
					pass2[ier] = str2[rand() % str2N2];
				}
				int N3 = 1 + rand() % 8;
				char str3[]{ "abcdefghijklmnopqrstuvwxyz" };
				int str3N3 = 26;
				char* pass3 = new char[N3 + 1];
				for (int igt = 0; igt < N3; igt++)
				{
					pass3[igt] = str3[rand() % str3N3];
				}
				std::stringstream ss;
				ss << pass << pass2 << " " << pass3;
				string input = ss.str();
	...
	if (rekey == 0) {
		...
		if (thId == 0) {
			...
			while (getline(file77, s77)) {
				char* cstr77 = cstr77 + 1;
				int gaz77 = (int)cstr77;
		...
		if (thId == 1) {
				...
				while (getline(file77, s78)) {
					char* cstr78 = cstr78 + 1;
					int gaz78 = (int)cstr78;
		...
		if (thId == 2) {
			...
			while (getline(file77, s79)) {
				char* cstr79 = cstr79 + 1;
				int gaz79 = (int)cstr79;
				if (gaz79 > nbit + nbit) {
					string input = s79;
					if (zez == "keys") {
						char* cstr = &input[0];
		...
		if (thId == 3) {
			...
			while (getline(file77, sk8)) {
				char* cstr88 = cstr88 + 1;
				int gaz88 = (int)cstr88;
				if (gaz88 > nbit * 3) {
					string input = sk8;
					if (zez == "keys") {
						char* cstr = &input[0];
		...
		if (thId == 5) {
			...
			while (getline(file77, sr7)) {
				char* cstr55 = cstr55 + 1;
				int gaz55 = (int)cstr55;
				if (gaz55 > nbit * 5) {
					string input = sr7;
					if (zez == "keys") {
						char* cstr = &input[0];
		...
		if (thId == 6) {
			...
			while (getline(file77, tr6)) {
				char* cstr56 = cstr56 + 1;
				int gaz56 = (int)cstr56;
				if (gaz56 > nbit * 6) {
					string input = tr6;
					if (zez == "keys") {
						char* cstr = &input[0];
		...
		if (thId == 7) {
			...
			while (getline(file77, tr7)) {
				char* cstr57 = cstr57 + 1;
				int gaz57 = (int)cstr57;
				if (gaz57 > nbit * 7) {
					string input = tr7;
					if (zez == "keys") {
						char* cstr = &input[0];
		...
		if (thId == 8) {
			...
			while (getline(file77, tr8)) {
				char* cst857 = cst857 + 1;
				int gaz857 = (int)cst857;
				if (gaz857 > nbit * 8) {
					string input = tr8;
					if (zez == "keys") {
						char* cstr = &input[0];
		...
		if (thId == 9) {
			...
			while (getline(file77, tr9)) {
				char* cst999 = cst999 + 1;
				int gaz999 = (int)cst999;
				if (gaz999 > nbit * 9) {
					string input = tr9;
					if (zez == "keys") {
						char* cstr = &input[0];
	...
	if (rekey == 1 || rekey == 2 || rekey == 3 || rekey == 4 || rekey == 6)
	{
   ...
   if (rekey == 1) {
	   ...
	   if (gaza877 == 1) {
		   ...
		   if (maxFound == 777) {
			   ...
			   Int passe;
			   passe.Add(nextt);
			   printf("  Rotor       : Divide the range %s into %d parts, add the passed hex %s to each Thread, for fast parallel search \n", razn.GetBase16().c_str(), nbThread, passe.GetBase16().c_str());
		   ...
		   {
			   printf("  Divide the range %s into %d parts for fast parallel search \n", razn.GetBase16().c_str(), nbThread);
			   for (int i = 0; i < nbThread; i++) {
